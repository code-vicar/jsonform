...
var _formElt;
var _expandOnClick = function () {
  // toggle class in html
  var el = $(this);
  var parent = el.parent();
  parent.toggleClass('expanded');
  $('> div', parent).slideToggle(100);

  // toggle state in formNode object
  while(!el.data('jsonformNode') && el.parent().length >= 1) {
    el = el.parent();
  }
  var node = el.data('jsonformNode');
  if (node) {
    node.expanded = !node.expanded;
  }
};

var initializeExpandable = function(formElt) {
  if (!formElt) {
    formElt = _formElt;
  }
  _formElt = formElt;
  $('.expandable:not(.expanded) > div, .expandable:not(.expanded) > fieldset', formElt).hide();
  $('.expandable.expanded > div, .expandable.expanded > fieldset', formElt).show();
  formElt.off('click', '.expandable > legend', _expandOnClick);
  formElt.on('click', '.expandable > legend', _expandOnClick);
}
 ...
 */
var initializeTabs = function (formElt) {
  if (!formElt) {
    formElt = _formElt;
  }
  _formElt = formElt;
...
jsonform.elementTypes = {
  ...
  'textarea':{
    'template':'<textarea id="<%= id %>" name="<%= node.name %>" ' +
      ...
      '<%= (node.required ? " required=\'required\'" : "") %>' +
      '<%= (node.placeholder? " placeholder=" + \'"\' + escape(node.placeholder) + \'"\' : "")%>' +
      '<%= (node.rows? " rows=" + \'"\' + escape(node.rows) + \'"\' : "")%>' +
      '<%= (node.cols? " cols=" + \'"\' + escape(node.cols) + \'"\' : "")%>' +
      '><%= value %></textarea>',
    'fieldtemplate': true,
    'inputfield': true
  },
  ...
  'fieldset':{
    'template': '' +
      '<fieldset class="jsonform-node jsonform-error-<%= keydash %> ' +
      '<% if (elt.expandable) { %>expandable<% } %> ' +
    ...
    'onInsert': function (evt, node) {
      if (evt.target.length) {
        evt.target.data('jsonformNode', node);
      }
    }
  },
  'advancedfieldset': {
  ...
  'selectfieldset': {
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %> <%= (node.isReadOnly() ? " jsonform-readonly" : "") %>">' +
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(children, function (child, idx) {
        tabs += '<option data-idx="' + idx + '" value="' + child.value + '"' +
          (child.node.active ? ' selected class="active"' : '') +
          '>' +
...
var formNode = function () {
  ...
  Object.defineProperty(this, 'expanded', {
    get: function() {
      ...
      return this.formElement.expanded;
    },
    set: function(val) {
      _expanded = val;
    }
...
formNode.prototype.isReadOnly = function() {
  return ((this &&
            ...
            this.ownerTree.formDesc.params.readOnly) || this.getProperty('readOnly', true));
}
...
formNode.prototype.switchValuesWith = function (node) {
  var values = this.getFormValues(node.arrayPath, true);
  var nodeValues = node.getFormValues(this.arrayPath, true);
...
formNode.prototype.resetValues = function () {
  var disabledInputs, params = null, idx = 0;
  ...
  if (this.view && this.view.inputfield) {
    ...
    // temporarily re-enable disabled inputs so their
    // values are reset as well.
    disabledInputs = $(':input:disabled', this.el)
 ...
 */
formNode.prototype.getFormValues = function (updateArrayPath, includeDisabled) {
  // The values object that will be returned
  var disabledInputs, values = {};
...
formNode.prototype.insertArrayItem = function (idx, domElement) {
  ...
  this.children[idx].expanded = true;
